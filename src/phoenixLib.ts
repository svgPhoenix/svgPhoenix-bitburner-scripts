...
export class botNetOperation {
  ...
  operation: 'hack' | 'weaken' | 'grow' = 'weaken';
  operationTime?: number;
  operationNumThreads?: number;
  timeUntilRetarget?: number;
}
/**
 * @param {NS} ns
...
export function findPathToServer(ns: NS, target: string, origin?: string) {
  ...
  //OPTI CPU: return [path, found] instead of checking path.includes constantly
  //OPTI RAM: prepend to path on the way up from the target instead of building and passing args
  function findPathRecursive(ns: NS, target: string, origin: string, path: string[], visited: string[]) {
    ...
    if (neighbors.includes(target)) {
      return path.concat([origin, target]);
    }
    for (let neighbor of neighbors) {
      if (visited.includes(neighbor)) {
        continue;
      }
      path = findPathRecursive(ns, target, neighbor, path.concat([origin]), visited);
