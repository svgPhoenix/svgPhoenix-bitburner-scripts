...
export function main(ns:NS) {
	const debug = ns.fileExists(".debug");
	ns.disableLog("scan");
	ns.clearLog();
	ns.tail();
...
function compressionII(compressed:string) {
	let decompressed = "";
	//true = "type A"
	let chunkType = true;
...
function algorithmicStocksI(prices:number[]) {
	let maxCur = 0;
	let maxSoFar = 0;
...
function encryptionI(data:[string, number]) {
	let List = [
...
function compressionI(data:string) {
